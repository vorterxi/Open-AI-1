...
const owners = ['@s.whatsapp.net'];

function sendMessage(jid, message) {
  Void.sendText(jid, message); 
}

for(let i of owners) {

  const readMessage = Config.readmessage === false ? 'Read Message: ❌' : Config.readmessage === true ? 'Read Message: ✅' : 'Read Message: ❌';

  const autoReadStatus = Config.auto_read_status === false ? 'Auto Read Status: ❌' : Config.auto_read_status === true ? 'Auto Read Status: ✅' : 'Auto Read Status: ❌';

  const disablePM = Config.disablepm === false ? 'Disable PM: ❌' : Config.disablepm === true ? 'Disable PM: ✅' : 'Disable PM: ❌';

  const openAIKey = Config.OPENAI_API_KEY === false ? 'OPENAI KEY: ❌' : Config.OPENAI_API_KEY === true ? 'OPENAI KEY: ✅' : 'OPENAI KEY: ❌';

  const levelUpMessage = Config.levelupmessage === false ? 'Level Up Message: ❌' : Config.levelupmessage === true ? 'Level Up Message: ✅' : 'Level Up Message: ❌';

  const autoReaction = Config.autoreaction === false ? 'Auto Reaction: ❌' : Config.autoreaction === true ? 'Auto Reaction: ✅' : 'Auto Reaction: ❌';

  sendMessage(i + '@s.whatsapp.net', {
