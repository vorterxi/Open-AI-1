...
for(let i of owner) {
  const read = Config.readmessage === false ? 'Read Message: ❌' : Config.readmessage === true ? 'Read Message: ✅' : 'Read Message: ❌';
  
  const ars = Config.auto_read_status === false ? 'Auto Read Status: ❌' : Config.auto_read_status === true ? 'Auto Read Status: ✅' : 'Auto Read Status: ❌';

  const dp = Config.disablepm === false ? 'Disable PM: ❌' : Config.disablepm === true ? 'Disable PM: ✅' : 'Disable PM: ❌';

  const oak = Config.OPENAI_API_KEY === false ? 'OPENAI KEY: ❌' : Config.OPENAI_API_KEY === true ? 'OPENAI KEY: ✅' : 'OPENAI KEY: ❌';

  const lap = Config.levelupmessage === false ? 'Level Up Message: ❌' : Config.levelupmessage === true ? 'Level Up Message: ✅' : 'Level Up Message: ❌';

  const ar = Config.autoreaction === false ? 'Auto Reaction: ❌' : Config.autoreaction === true ? 'Auto Reaction: ✅' : 'Auto Reaction: ❌';
